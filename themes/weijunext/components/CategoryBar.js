...
export default function CategoryBar(props) {
  ...
  ${
    border
      ? 'lg:border lg:hover:border dark:lg:border-gray-800 hover:border-indigo-600 dark:hover:border-yellow-600 '
      : ''
    ...
    >
      ...
      >
        ...
        {categoryOptions?.map((c, index) => (
          <MenuItem key={c.id} href={`/category/${c.name}`} name={c.name} />
        ))}
      </div>
...
const MenuItem = ({ href, name }) => {
  ...
  const selected = decodeURIComponent(router.asPath) === href
  return (
    <div
      className={`whitespace-nowrap mr-2 duration-200 transition-all font-bold px-2 py-0.5 rounded-md text-gray-900 dark:text-white hover:text-white hover:bg-indigo-600 dark:hover:bg-yellow-600 ${
        selected ? 'text-white bg-indigo-600 dark:bg-yellow-600' : ''
      }`}
    >
